...
from .commit_stream import GitCommit
from .github_api import github_info_by_emails
...
class Contributor:
    ...
    def add_commit(self, commit: GitCommit):
        """Add a commit to the contributor's history.

        Args:
            commit: The GitCommit object to add.
        """
        if self.last_commit_ts is None or self.last_commit_ts < commit.ts:
            self.last_commit_ts = commit.ts
...
class ContributorCollection:
    ...
    def save_to_file(self):
        ...
        with open(self.db_filename, "w") as out_file:
            yaml.safe_dump_all(
                self.contributors,
                out_file,
                indent=2,
    ...
    def load_from_file(self):
        ...
        try:
            with open(self.db_filename, "r") as in_file:
                for item in yaml.safe_load_all(in_file):
