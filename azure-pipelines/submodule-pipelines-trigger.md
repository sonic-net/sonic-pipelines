...
|-------|----------------------------------|
| prerequisites | `docker_slave_trixie` (3006), `docker_slave_bookworm` (1148) |
...
> `docker_slave_trixie`, `docker_slave_bookworm`, and `common_libs` are buildimage
> pipelines rather than submodule gitlinks, so they use the buildimage commit itself
> (`git rev-parse HEAD`). All other entries resolve their pinned commit via
